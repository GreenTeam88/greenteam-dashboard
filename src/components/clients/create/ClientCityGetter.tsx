'use client';

import { MapPin } from 'lucide-react';
import { memo } from 'react';

import CustomInput from '@/components/custom/CustomInput';
import { FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';

interface ClientCityGetterProps {
  form: any;
}

function ClientCityGetter({ form }: ClientCityGetterProps) {
  return (
    <FormField
      control={form.control}
      name={'city'}
      render={({ field }) => (
        <FormItem className={'flex flex-col gap-y-[0.875rem]'}>
          <FormLabel className={'font-normal text-textBlack80 text-sm'}>City</FormLabel>
          <FormControl>
            <div className={'!m-0 relative'}>
              <CustomInput
                value={field.value ?? ''}
                onChange={(value) => {
                  form.setValue('city', value);
                }}
                className={
                  '!ring-transparent !outline-transparent h-auto border py-3 px-5 rounded-lg border-borderGray text-sm text-textBlack40'
                }
                type={'text'}
                placeholder={'Input city'}
              />
              <div className={'absolute top-0 right-0 h-full flex justify-center items-center mr-2'}>
                <MapPin className={'size-4 font-normal text-textBlack'} />
              </div>
            </div>
          </FormControl>
          <FormMessage />
        </FormItem>
      )}
    />
  );
}

export default memo(ClientCityGetter);
