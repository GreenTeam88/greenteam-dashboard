...
function ClientHouseNumberGetter({ form }: ClientHouseNumberGetterProps) {
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem className={'flex flex-col gap-y-[0.875rem]'}>
          <FormLabel className={'font-normal text-textBlack80 text-sm'}>Housenumber</FormLabel>
          <FormControl>
            <CustomInput
              value={field.value ?? ''}
              onChange={(value) => {
...
export default memo(ClientHouseNumberGetter);
